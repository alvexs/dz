...
def flight(request, id):
    ...
    if request.method == 'POST':
        ...
        if state == "True" and not \
                flight.participation.filter(id=request.user.id).exists():
        ...
        return render_to_response('users_list.html',
                                  {'users': flight.participation.all()})
...
def ajax_list(request, page_id):
    pos = int(page_id)
    return render_to_response('object_container.html',
                              {'flights': render_cons(pos)})
